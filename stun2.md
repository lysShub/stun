...
| 对称形       | IP限制锥形   | ╳✓   | 与IP限制锥形-端口限制锥形类似。S、R请求SV、SV获取到S、R的网关地址，SV控制R向S的网关IP的任意端口发送任意数据包，此时R的网关将接收来自S网关的任意端口的数据包；SV又控制S网关的网关端口发送数据包，；R获取到数据包、也获取到S的地址，R回复S，S收到这个数据包，即完成穿隧。（当S请求SV和R时的网关IP不同时不能成功） |
| 对称形       | 端口限制锥形 | ╳    |                                                              |
| 对称形       | 对称形       | ╳    |                                                              |
...
​		以上穿隧情况是细化的，我们实现时可以更加统一化：双方先请求SV，得到NAT类型并比较S和R的网关NAT的严格性；(1)、更宽松一方首次请求（同时也接收）更严格的一方的多个端口，包括SV获取到的端口及附近几个端口（端口猜测）。(2)、在执行步骤1后，更严格的一方也请求（同时也接收）更宽松一方的多个端口。(3)、如果更宽松的一方收到了对应一个数据包，则会在对应的接口回复更宽松的一方；此时穿隧完成。

说明：(1)、SV获取到的端口及S或R请求SV时，SV得到的请求者的NAT网关端口。

​			(2)、如果3没有收到对应的数据包，则表示无法完成传输。

​			(3)、请求多个端口的实际实现是先请求SV获取到的端口，延时一段时间后再猜测端口。

​			(4)、对于双方网关一样的情况，我们指定S为网关宽松一方。
...
| 2                          | SV     | S和R   | Tuuid:2:x      | SV回复，x为0或1，0为低的一方。                               |
| <font color="red">3</font> | 低     | 高     | Tuuid:3:i      | 低的一方请求高的一方，发送之后将接收，如果接收到6则穿隧成功  |
| <font color="red">4</font> | 低     | SV     | Tuuid:4:i      | 低的一方请求服务器，服务器获取网关端口后告知高的一方(5)      |
| <font color="red">5</font> | SV     | 高     | Tuuid:5:port:i | SV通知高的一方，之后低的一方将请求高的一方，请求端口是相邻的端口，此port端口是4过程获取到的低的网关端口 |
| --                         | --     | --     | --             | ----如果穿隧没有成功，可一定重复3-5过程，i累加----           |
| 6                          | 高     | 低     | Tuuid:6        |                                                              |

说明：

- selfAddr即己方网关地址，otherAddr即对方网关地址；长度为6个字节、前4个为IP、后两个为端口。
