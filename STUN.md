...
| 序号 | 发送方 | 接收方 | data                       | 说明                                                         |
| ---- | ------ | ------ | -------------------------- | ------------------------------------------------------------ |
| 1    | S和R   | SV     | Tuuid:1                    | 双方Tuuid要相同，SV将记录双方网关地址                        |
| 2    | SV     | S和R   | Tuuid:2:selfAddr:otherAddr | SV回复                                                       |
| 3    | S和R   | S和R   | Tuuid:3                    | 无论哪一方，一旦接收到3，即立即回复，并停止发送数据并关闭其他Socket |
| 4    | S或R   | S或R   | Tuuid:4                    | 步骤3的回复，接收到此数据包，将停止发送数据并关闭其他Socket，同时将回复5 |
| 5... | S或R   | S或R   | Tuuid:n                    | 无论哪一方，接收到5及其以上的数据包时，都回复n+1数据包，用于验证通道可靠；当n大于等于20时即可返回Socket句柄 |

说明：selfAddr即己方网关地址，otherAddr即对方网关地址；长度为6个字节、前4个为IP、后两个为端口。
