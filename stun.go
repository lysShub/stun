...
func (s *STUN) init() error {
	if s.SeconNetCardIP != nil {
		l, err := net.ResolveUDPAddr("udp", s.SeconNetCardIP.String()+":"+strconv.Itoa(int(s.Port)))
		if err != nil {
			return nil
		}
		s.seconConn, err = net.ListenUDP("udp", l)
		if err != nil {
			s.seconConn = nil
			return err
		}
...
func (s *STUN) DiscoverClient() (int16, error) {
	...
	//  e Sever no response
	//  f Exceptions
	raddr1, err1 := net.ResolveUDPAddr("udp", s.SeverAddr+":"+strconv.Itoa(int(s.Port)))
	laddr1, err2 := net.ResolveUDPAddr("udp", ":"+strconv.Itoa(int(s.Port)))
	laddr2, err3 := net.ResolveUDPAddr("udp", ":"+strconv.Itoa(int(s.SecondPort)))
...
//
func (s *STUN) DiscoverSever() error {
